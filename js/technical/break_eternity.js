...
; (function (globalScope) {
  ...
  function getConst(name) {
    return ExpantaNum[name];
  }

  // ===== 2. 优化的 P.add =====
  P.plus = P.add = function (other) {
    // 快速路径：如果 other 是 0，直接返回 this
    if (other === 0) {
      return this.clone();
    }

    var x = this.clone();
    var y = (other instanceof ExpantaNum) ? other : new ExpantaNum(other);
    ...
    // 使用优化后的 eq 方法判断零值
    var ZERO = getConst("ZERO");
    if (x.eq(ZERO)) return y.clone();
    if (y.eq(ZERO)) return x.clone();

    // 特殊值处理
    var NaNConst = getConst("NaN");
    if (x.isNaN() || y.isNaN()) return NaNConst.clone();

    var xInf = x.isInfinite();
    var yInf = y.isInfinite();
    if (xInf || yInf) {
      if (xInf && yInf && x.eq(y.neg())) return NaNConst.clone();
      return xInf ? x : y;
    }

    // 大小比较
    var cmpResult = x.cmp(y);
    var p = cmpResult <= 0 ? x : y;  // min
    var q = cmpResult >= 0 ? x : y;  // max

    var op0 = q.operator(0);
    var op1 = q.operator(1);
    var t;

    var E_MAX_SAFE_INTEGER = getConst("E_MAX_SAFE_INTEGER");
    var MAX_SAFE_INTEGER = getConst("MAX_SAFE_INTEGER");

    if (q.gt(E_MAX_SAFE_INTEGER) || q.div(p).gt(MAX_SAFE_INTEGER)) {
      t = q.clone();
    } else if (!op1) {
      // 安全整数范围内的加法
      t = new ExpantaNum(x.toNumber() + y.toNumber());
    } else if (op1 === 1) {
  ...
  P.minus = P.sub = function (other) {
    ...
    var yInf = y.isInfinite();
    if (xInf || yInf) {
      if (xInf && yInf) return NaNConst.clone();
      return xInf ? x : y.neg();
    }

    // 比较和计算
    var cmpResult = x.cmp(y);
    var n = cmpResult < 0; // y > x
    var q = n ? y : x; // max
    var p = n ? x : y; // min
    ...
    if (q.gt(E_MAX_SAFE_INTEGER) || q.div(p).gt(MAX_SAFE_INTEGER)) {
      ...
      var a = p.operator(1) ? p.operator(0) : Math.log10(p.operator(0));
      var diff = a + Math.log10(Math.pow(10, op0 - a) - 1);
      t = new ExpantaNum([diff, 1]);
      if (n) t = t.neg();
    }
  ...
  P.times = P.mul = function (other) {
    ...
    // 符号处理优化
    var signProduct = x.sign * y.sign;
    if (signProduct === -1) {
      return x.abs().mul(y.abs()).neg();
    }
    if (x.sign === -1) {
      return x.abs().mul(y.abs());
    }

    // 特殊值处理
    var NaNConst = getConst("NaN");
    var ZERO = getConst("ZERO");
    var ONE = getConst("ONE");

    if (x.isNaN() || y.isNaN()) return NaNConst.clone();
    if ((x.eq(ZERO) && y.isInfinite()) || (x.isInfinite() && y.abs().eq(ZERO))) {
      return NaNConst.clone();
    }
    if (y.eq(ZERO)) return ZERO.clone();
    if (y.eq(ONE)) return x.clone();
    if (x.isInfinite()) return x;
    if (y.isInfinite()) return y;

    // 大小判断优化
    var maxVal = x.max(y);
    var EE_MAX_SAFE_INTEGER = getConst("EE_MAX_SAFE_INTEGER");
    if (maxVal.gt(EE_MAX_SAFE_INTEGER)) {
      return maxVal.clone();
    }

    // 数值计算优化
    var n = x.toNumber() * y.toNumber();
    if (n <= MAX_SAFE_INTEGER) {
      return new ExpantaNum(n);
    }

    // 大数乘法：10^(log10(x) + log10(y))
    return ExpantaNum.pow(10, x.log10().add(y.log10()));
  };
  ...
  P.divide = P.div = function (other) {
    ...
    var signProduct = x.sign * y.sign;
    if (signProduct === -1) {
      return x.abs().div(y.abs()).neg();
    }
    if (x.sign === -1) {
      return x.abs().div(y.abs());
    }
    ...
    var ZERO = getConst("ZERO");
    var ONE = getConst("ONE");
    var POSITIVE_INFINITY = getConst("POSITIVE_INFINITY");

    if (x.isNaN() || y.isNaN()) return NaNConst.clone();
    if ((x.isInfinite() && y.isInfinite()) || (x.eq(ZERO) && y.eq(ZERO))) {
      return NaNConst.clone();
    }
    if (y.eq(ZERO)) return POSITIVE_INFINITY.clone();
    if (y.eq(ONE)) return x.clone();
    if (x.isInfinite()) return x;
    if (y.isInfinite()) return ZERO.clone();

    // 大小判断优化
    var maxVal = x.max(y);
    var EE_MAX_SAFE_INTEGER = getConst("EE_MAX_SAFE_INTEGER");
    if (maxVal.gt(EE_MAX_SAFE_INTEGER)) {
      return x.gt(y) ? x.clone() : ZERO.clone();
    }
    ...
    var pw = ExpantaNum.pow(10, x.log10().sub(y.log10()));
    var fp = pw.floor();
    // 优化：直接比较差值，避免创建新对象
    if (pw.sub(fp).lt(new ExpantaNum(1e-9))) {
      return fp;
    }
    return pw;
  };
